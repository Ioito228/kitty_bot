...
async def show_pet_home(m_cb, data):
    ...
    emotion = p.get_emotion()
    img_path = config.IMAGE_PATHS[gender][emotion]
...
async def cmd_start(m: types.Message, state: FSMContext):
    ...
    if data: await show_pet_home(m, data)
        ...
        await m.answer("Привет! Давай заведем тебе питомца. Как его назовем?")
...
async def math_start(cb: types.CallbackQuery, state: FSMContext):
    ...
    img_playing = config.IMAGE_PATHS[raw[1]]["playing"] 
    
    a, b = random.randint(10, 60), random.randint(10, 60)
    await state.update_data(math_res = a + b)
    await cb.answer()
    await send_optimized_photo(cb.message, img_playing, f"🧠 Реши пример: {a} + {b} = ?")
...
async def math_logic(m: types.Message, state: FSMContext):
    sd = await state.get_data(); raw = await db.load_pet_data(m.from_user.id); p = Pet(*raw[:5], raw[6])
    if m.text == str(sd.get("math_res")):
        p.mood = min(100, p.mood + 15); txt = "✅ Верно! Настроение +15"
    else:
        txt = f"❌ Нет, правильный ответ {sd.get('math_res')}"
...
async def ana_start(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.answer()
    await send_optimized_photo(cb.message, img_playing, f"🔤 Собери слово: **{shuffled}**")
...
async def ana_logic(m: types.Message, state: FSMContext):
    sd = await state.get_data(); raw = await db.load_pet_data(m.from_user.id); p = Pet(*raw[:5], raw[6])
    if m.text.lower().strip() == sd.get("ana_ans"):
        p.mood = min(100, p.mood + 20); txt = "✅ Правильно! Настроение +20"
    else:
        txt = f"❌ Ошибка, это было слово {sd.get('ana_ans')}"
    await db.save_pet(m.from_user.id, p, raw[5]); await state.clear(); await m.answer(txt, reply_markup=main_kb())
...
async def btn_chat(cb: types.CallbackQuery, state: FSMContext):
    await cb.answer(); await cb.message.answer("Пиши питомцу (или 'Стоп'):"); await state.set_state(PetStates.chatting)

@dp.message(PetStates.chatting)
async def chat_proc(m: types.Message, state: FSMContext):
    if m.text.lower() in ["стоп", "выход"]: await state.clear(); await m.answer("Ок!", reply_markup=main_kb()); return
...
async def city_btn(cb: types.CallbackQuery, state: FSMContext):
    await cb.answer(); await cb.message.answer("Введи город:"); await state.set_state(PetStates.changing_city)

@dp.message(PetStates.changing_city)
async def city_logic(m: types.Message, state: FSMContext):
    raw = await db.load_pet_data(m.from_user.id); p = Pet(*raw[:5], raw[6])
    await db.save_pet(m.from_user.id, p, m.text); await state.clear(); await m.answer(f"Город: {m.text}", reply_markup=main_kb())

@dp.message(PetStates.naming)
async def naming(m: types.Message, state: FSMContext):
    await state.update_data(n=m.text); await state.set_state(PetStates.choosing_gender)
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="👦 Мальчик", callback_data="sex_boy"), InlineKeyboardButton(text="👧 Девочка", callback_data="sex_girl")]])
    await m.answer(f"Пол для {m.text}?", reply_markup=kb)

@dp.callback_query(PetStates.choosing_gender)
async def choosing_sex(cb: types.CallbackQuery, state: FSMContext):
    g = "boy" if "boy" in cb.data else "girl"; sd = await state.get_data(); p = Pet(sd['n'], g)
    await db.save_pet(cb.from_user.id, p, "Москва"); await state.clear(); await cb.message.answer("Готово!", reply_markup=main_kb())
...
async def time_loop():
    while True:
        ...
        try:
            ...
            for uid in uids:
                raw = await db.load_pet_data(uid); p = Pet(*raw[:5], raw[6])
                imp, _ = await weather.get_weather(raw[5]); p.time_passes(imp)
                if p.health <= 0: await db.delete_pet(uid)
                else: await db.save_pet(uid, p, raw[5])
        except: pass

async def main():
    await db.init_db(); asyncio.create_task(time_loop()); await bot.delete_webhook(drop_pending_updates=True); await dp.start_polling(bot)
